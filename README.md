...
投影法切り替え時のアニメーション動作は、JavaScriptで以下のパラメータを調整できます（`js/ui-controls.js`）:

#### 基本タイミング
- **`transitionDuration`** (16行目): `3000` ミリ秒
  - メインのトランジションアニメーション時間
  - 大きくするとゆっくり、小さくすると速く遷移します

#### 物理シミュレーション（バネの運動）
- **`springStrength`** (23行目): `0.38`
  - バネの強さ。目標値へ向かう力の大きさ
  - **大きくする** (例: 0.5) → 速く到達、振動が大きい
  - **小さくする** (例: 0.2) → ゆっくり到達、振動が小さい
  - 推奨範囲: 0.1-0.5

- **`damping`** (24行目): `0.95`
  - 速度の減衰率。毎フレーム速度に掛けられます
  - **大きくする** (例: 0.98) → 振動が多い、よく行き過ぎる
  - **小さくする** (例: 0.85) → 振動が少ない、すぐに安定
  - 推奨範囲: 0.85-0.98

#### 動作原理
投影法を切り替える際、以下のアニメーション順序で実行されます：
1. **easeInOutBack** で目標値を決定（overshootが含まれる）
2. **バネの物理演算** で目標値に向かって現在値を更新
   - `velocity += (targetProgress - currentProgress) × springStrength`
   - `velocity × damping`で減衰
   - `currentProgress += velocity`で位置を更新
3. 減衰により徐々に静止

これにより、「目標値に向かってぷよぷよ振動しながら到達」する自然な動きになります。
...
- 🌊 物理シミュレーション（バネの運動）を実装
  - springStrength と damping パラメータで細かく調整可能
  - 目標値に向かってぷよぷよ振動しながら到達する自然な動き
  - `js/ui-controls.js` で設定値を変更可能
